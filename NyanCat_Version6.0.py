...
#Date Last Modified: 14 July 2013
""" 
  ...
  Version 6.0 - *     adding instructions state that works as end state. Before the game start user 
                      have to go to instruction state then when the user click the mouse it goes to
                      the game. when user lose the game the score get printed at instruction/end state 
                      and user can have options to continue the game or quit the game.
...
class NyanCat(pygame.sprite.Sprite):
    def __init__(self):
        ...
        if not pygame.mixer:
            ...
            pygame.mixer.init()
            self.sndBaloon = pygame.mixer.Sound("Nyan-baloon.ogg") #sounds when collide with nyan balloon
            self.sndTac = pygame.mixer.Sound("tac-nyan.ogg") #sounds when collide with Tac Nyan
...
def instructions(score):
    ...
    "Tac Nyan is your number one enemy and very hard to defeat",
    "avoid Tac Nyan to stay alive",
