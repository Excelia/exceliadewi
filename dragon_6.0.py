...
#Revision History: do minor change such as the version file and print text
...
#this is the first decision level that user has to choose    
def chooseRoad():
    road = ''
    while road != '1' and road != '2':
        time.sleep(1) #elapse time between each line that printed
        print ('Which road you will choose 1=rocky road or 2=sandy road? (1 or 2)')
        road = raw_input() #take the user input
    return road

#check if user input the first or second value then some event going to happend
def checkRoad(chosenRoad): 
    ...
    #the result of first decision level, user has to choose between jungle and mountain OR lake and river
    if chosenRoad == "1":
...
#function that will handle if user choose to take lake and river
#the second decision level stage
#the outcome from first decision level stage
def lakeAndRiver():
    ...
    while pick_path !='1' and pick_path != '2':
            ...
            break
    pick_path = raw_input('')
    #this is the outcome from second decision level stage
    if pick_path == "1":
...
#this function handle if the user choose jungle and mountain
...
#this function handle if user choose the river path
#third decision level stage
def riverPath():
    print('youre crossing the river now')
    time.sleep(1)
    ...
    #the outcomes from the third decision level stage
    if choose_action == '1':
...
#this function handle if user take the lake path
def lakePath():
...
#this function handle if the user take the jungle path
def junglePath():
...
#this function handle if the user take the mountain path
def mountainPath():
...
def main():
    ...
    #if the games over it will prompt the user to play the games again or no
    playAgain = 'yes'
    while playAgain == 'yes' or playAgain == 'y':
        displayIntro() #if user say yes it will display the intro story 
        roadNumber = chooseRoad() #and restart the game again
